...
@step
def make_dataset(params: InferencePipelineParams, df: pd.DataFrame) -> data.FinancialTweetsDataset:
    return data.FinancialTweetsDataset(df[data.text_column], df[data.has_source_column], df[data.label_column], data.get_tokenizer(params.tokenizer_name), params.max_length)

@step
def predict(params: InferencePipelineParams, dataset: data.FinancialTweetsDataset) -> str:
